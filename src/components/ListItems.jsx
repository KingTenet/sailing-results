...
function ResultDimension({ children, ...props }) {
  return (
    <GridItem height="20px" maxWidth="100%" {...props}>
...
function Slider({
  ...
  const [clickDisabled, updateClickDisabled] = useState(true);
  ...
  const bind = useDrag(
    ({ active, movement: [x, y], last }) => {
      ...
      const xToUse = Math.max(
        Math.min(x, onDelete ? MAX_X : 0),
        onDNF ? -MAX_X : 0
      );
      if (
        last &&
      ...
      if (clickDisabled && xToUse === 0) {
        updateClickDisabled(false);
      } else if (!clickDisabled && xToUse !== 0) {
        updateClickDisabled(true);
      }
    ...
    <div className="container">
      <animated.div className={"item"}>
        <div className={"item slider-background"}>
          <animated.div
            className={"av"}
            style={{ scale: avSize, justifySelf: "start", lineHeight: "35px" }}
          >
        ...
        <animated.div
          ...
          style={{
            x,
            opacity: isActive.to((v) => (v ? 1 : 0)),
            touchAction: "pan-y",
          }}
        >
          <div className="active item">
            <Box className="list-item" as="button" textAlign="left">
              <Flex direction={"row"} justifyContent="space-between">
                {onDelete ? <ArrowForwardIcon boxSize={"5"} /> : <div />}
                <Spacer />
                {onDNF ? <ArrowBackIcon boxSize={"5"} /> : <div />}
              </Flex>
            </Box>
          </div>
        </animated.div>
...
function ListItemWrapper({
  ...
  return (
    ...
    >
      <Box className="list-item" as="button" textAlign="left" onClick={onClick}>
        <Flex direction={"row"} justifyContent="space-between">
          {children}
          <Flex
            direction={"row"}
    ...
    <>
      <ListItemWrapper {...props}>
        <Grid templateColumns="repeat(17, 1fr)" gap={3} width={"100%"}>
          ...
          {validFinish && (
            <ResultDimension colSpan={6}>{`${laps} lap${
              laps > 1 ? "s" : ""
            } in ${finishTime}`}</ResultDimension>
          )}
          {!validFinish && (
            <ResultDimension colSpan={6}>{"DNF"}</ResultDimension>
          )}
        </Grid>
      </ListItemWrapper>
    </>
