...
function DraggableView({
  ...
  const onDNF = (registeredToMove) =>
    updateRaceResults(
      registered.filter(
        (prev) => HelmResult.getId(prev) !== HelmResult.getId(registeredToMove)
      ),
      finished,
  ...
  const resetFinisher = (itemToReset) => {
    updateRaceResults(
      ...
      finished.filter(
        (prev) => HelmResult.getId(prev) !== HelmResult.getId(itemToReset)
      ),
      dnf.filter(
        (prev) => HelmResult.getId(prev) !== HelmResult.getId(itemToReset)
      ),
    ...
    return (
      <Flex direction="column" className="device-height" alignItems={"center"}>
        {(editingRace || !raceIsMutable) && (
          ...
          >
            <Heading size={"lg"} marginLeft="20px">{`${getURLDate(
              raceDate
            ).replace(/-/g, "/")}`}</Heading>
            <Spacer />
            <Heading size={"lg"} marginRight="20px">{`${formatRaceNumber(
              raceNumber
            )} ${formatFleetPursuit(isPursuitRace)} race`}</Heading>
          </Flex>
        )}
        {children}
        {!raceIsMutable && (
    ...
    return (
      <Wrapped>
        ...
        {isPursuitRace && raceRegistered.length + raceResults.length > 1 && (
          <GreenButton onClick={() => updateShowStartTimes(!showStartTimes)}>
      ...
      <>
        {!Boolean(
          ...
          <CopyFromPreviousRace
            race={race}
            previousRace={new StoreRace(race.getDate(), race.getNumber() - 1)}
          ></CopyFromPreviousRace>
        )}
        <Wrapped>
          <DraggableView
