  ...
  useEffect(() => {
    ...
    if (exactMatch === undefined && forceBlurOnExactMatch) {
      let exactMatch = data.find(
        (item) =>
          itemToString(item).toLowerCase() === partialMatch.toLowerCase()
      );
      if (exactMatch) {
      ...
      <div className="space-y-2">
        <div className="flex gap-2 items-center">
          ...
          <div className="flex-1">
            <Input {...getInputProps()} placeholder={placeholder} type={type} />
          </div>
        </div>
          ...
          <ul {...getMenuProps()} className="mt-2 space-y-1">
            {getMenuItems(inputItems, data)
              ...
              .sort(([itemA, indexA], [itemB, indexB]) =>
                sortFn && !partialMatch ? sortFn(itemA, itemB) : indexA - indexB
              )
              .map(([item, index]) => (
                <li
