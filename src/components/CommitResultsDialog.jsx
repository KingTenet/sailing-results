...
export default function CommitResultsDialog({
  race,
  onSuccess,
  onFailed,
  onStarted,
    ...
    return services
      ...
      .then((helmIdsRemoved) =>
        updateAppState(({ newHelms, ...state }) => ({
          ...state,
          newHelms: newHelms.filter(
            (newHelm) => !helmIdsRemoved.includes(Helm.getId(newHelm))
          ),
        }))
      );
  };
      ...
      .then(() => {
        updateAppState(({ results, registered, oods, ...state }) => ({
          ...
          registered: registered.filter(
            (registered) => !raceRegistered.includes(registered)
          ),
  ...
  return (
    <AlertDialogWrapper
      ...
      deleteHeading={
        raceOODs.length
          ? `Are you sure you want to commit results?`
          : `No race officers have been registered. Are you sure you want to commit results?`
      }
