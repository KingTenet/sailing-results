...
const NEW_HELM_GENDERS = ["Male", "Female"];

const NOVICE = "a Novice";

const NEW_HELM_EXPERIENCE = [NOVICE, "an Experienced"];
...
export default function NewHelm({ clubMember, onNewHelm, label }) {
    ...
    const onConfirm = () => {
        ...
        if (genderEnabled && experienceEnabled) {
            onNewHelm(
                services.createHelmFromClubMember(
                    clubMember,
                    getGender(gender),
                    experience === NOVICE,
                ),
                ...
                {!confirmed && (
                    <Box minWidth="110px" paddingTop="10px">
                        <Alert status="error">
                            ...
                            <AlertTitle
                                mr={2}
                            >{`${clubMember.getName()} has not raced before. Please provide some additional details.`}</AlertTitle>
                        </Alert>
                    </Box>
                )}
                <Flex direction={"row"} marginTop="20px">
                    <Text fontSize={"large"} marginTop="7px">
                        {clubMember.getName()} is
                    </Text>
                    {!experienceEnabled && (
                        <BlueButton
                    ...
                    <Text fontSize={"large"} marginTop="7px" marginLeft="10px">
                        sailor
                    </Text>
                    {confirmed && (
                        <CheckCircleIcon
