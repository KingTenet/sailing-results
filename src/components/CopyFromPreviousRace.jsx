...
export default function CopyFromPreviousRace({ race, previousRace }) {
  ...
  const raceResults = appState.results.filter(
    (result) => HelmResult.getRaceId(result) === StoreRace.getId(previousRace)
  );
  const raceRegistered = appState.registered.filter(
    (result) => HelmResult.getRaceId(result) === StoreRace.getId(previousRace)
  );
  const oods = appState.oods.filter(
    (ood) => HelmResult.getRaceId(ood) === StoreRace.getId(previousRace)
  );
  ...
  const [[previousRaceResults, previousRaceOODs]] = useState(() => {
    ...
    const previousRaceValid =
      !viewableRaceResults.length ||
      viewableRaceResults.some((result) => result.finishCode.validFinish());
    const previousRaceFinish =
      previousRaceValid &&
      services.getRaceFinishForResults(previousRace, viewableRaceResults, oods);

    return [
      (previousRaceFinish &&
        previousRaceFinish.results.length &&
        previousRaceFinish.getCorrectedResults()) ||
        [],
  ...
  const { isOpen, onClose } = useDisclosure({
    defaultIsOpen: previousRaceResults.length || previousRaceOODs.length,
  });
  ...
  const copyHelmsOODs = () => {
    updateAppState(({ registered, oods, ...state }) => ({
      ...
      registered: [
        ...registered,
        ...previousRaceResults.map((result) =>
          MutableRaceResult.fromPreviousResult(result, race)
        ),
      ],
      oods: [
        ...oods,
        ...previousRaceOODs.map((result) =>
          HelmResult.fromPreviousResult(result, race)
        ),
      ],
    }));
  };
