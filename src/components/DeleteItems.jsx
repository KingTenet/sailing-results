  ...
  const deleteFinisher = () => {
    updateAppState(({ results, ...state }) => ({
      ...
      results: results.filter(
        (result) => HelmResult.getId(result) !== HelmResult.getId(finisher)
      ),
    }));
  };
...
export function DeletePursuitFinish({
  ...
  const deletePursuitFinish = () => {
    updateAppState(({ registered, ...state }) => ({
      ...
      registered: registered.filter(
        (prev) =>
          HelmResult.getId(prev) !== HelmResult.getId(registeredToDelete)
      ),
    }));
  };
  ...
  return (
    <AlertDialogWrapper
      ...
      deleteHeading={`Delete helm: ${HelmResult.getHelmId(
        registeredToDelete
      )}.`}
    >
      {children}
