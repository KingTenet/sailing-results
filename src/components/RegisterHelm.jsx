...
function RegisterHelm({ addAnotherHelmWorkflow }) {
    ...
    const processHelmResult = useCallback(() => {
        if (!sailNumber) {
            return;
        }
            ...
            if (
                registered.find(
                    (prev) =>
                        ...
                        HelmResult.getId(newRegisteration),
                ) ||
                results.find(
                    (prev) =>
                        HelmResult.getId(prev) ===
                ...
                return {
                    ...
                    results: results,
                    oods: oods,
                    registered: [...registered, newRegisteration].sort(
                        (helmA, helmB) => {
                            const helmAPI =
                                services.getLatestHelmPersonalHandicap(
                                    HelmResult.getHelmId(helmA),
                                );
                            const helmBPI =
                                services.getLatestHelmPersonalHandicap(
                                    HelmResult.getHelmId(helmB),
                                );
                            return helmBPI > helmAPI || helmBPI === undefined
                                ? -1
                                : 1;
                        },
                    ),
        ...
        if (!addAnotherHelmWorkflow) {
            ...
            navigateTo(
                `/races/${getURLDate(race.getDate())}/${race.getNumber()}/registerAnother/${HelmResult.getHelmId(newRegisteration)}`,
                { replace: true },
            );
        }
    },[services, race, selectedHelm, selectedBoat, sailNumber, appState.newHelms, updateAppState, addAnotherHelmWorkflow, navigateBack, navigateTo]);
    ...
    useEffect(() => {
        if (selectedHelm) {
            setBoatsIndex(
                services.indexes.getBoatIndexForHelmRace(selectedHelm, race),
            );
        } else {
    ...
    const handleSelectedCrew = (selectedCrew) => {
        if (selectedCrew instanceof ClubMember) {
            // Do some club member stuff..
            setSelectedCrewClubMember(selectedCrew);
        } else {
    ...
    const onNewHelmName = (newMemberName) => {
        try {
            ...
            if (
                !services.stores.clubMembers.has(cleanedName) &&
    ...
    const { onOpen: onNewCrewOpen, isOpen: newCrewIsOpen, onClose: onNewCrewClose } = useDisclosure();
    const onNewCrewName = (newMemberName) => {
        try {
            const cleanedName = cleanName(newMemberName);
            if (
                !services.stores.clubMembers.has(cleanedName) &&
                ...
                setSelectedCrewNewMember(cleanedName);
                onNewCrewOpen();
            }
        } catch (err) {
            // do nothing
    ...
    const onConfirmNewCrewClubMember = () => {
        if (services.stores.helms.has(selectedCrewNewMember)) {
            setSelectedCrew(services.stores.helms.get(selectedCrewNewMember));
            return;
        }
        setSelectedCrewClubMember(ClubMember.fromName(selectedCrewNewMember));
    };
                    ...
                    >
                        ...
                        {selectedHelm && boatsIndex && (
                            <Autocomplete
                                ...
                                itemToString={(boat) =>
                                    boat ? boat.getClassName() : ""
                                }
                                filterData={(inputValue) =>
                                    boatsIndex.search(inputValue)
                                }
                                handleSelectedItemChange={setSelectedBoat}
                                getPartialMatchErrorMsg={(partialMatch) =>
                                    `${partialMatch} is not a recognised boat class`
                                }
                        ...
                        {crewNumber && crewNumber > 1 && helmsIndex && !selectedCrewClubMember && showSelectedCrew && crewIndex &&(
                            <AlertDialogWrapper
                                ...
                                confirmButtonText="Confirm"
                                warningText="Do you want to add them?"
                                deleteHeading={`${selectedCrewNewMember} is not a recognised member.`}
                            >
                                <Autocomplete
                                    customClassName="input-container-1 input-container"
                                    heading={"Crew"}
                                    ...
                                    placeholder={"Enter crew name here..."}
                                    handleOnBlur={onNewCrewName}
                                    forceBlurOnExactMatch={true}
                                    getPartialMatchErrorMsg={
                                        getHelmNameErrorMessage
                                    }
                        ...
                        {selectedBoat && !needCrewSelection && sailNumberIndex && (
                            <>
                                <Autocomplete
                                    ...
                                    filterData={(inputValue) => {
                                        const matches =
                                            sailNumberIndex.search(inputValue);
                                        if (
                                            !matches.length &&
                        ...
                        {(sailNumber !== undefined || showCommit) && (
                            <GreenButton
                                onClick={() => processHelmResult()}
                                autoFocus={sailNumber}
                            >
                                Register Helm
                            </GreenButton>
                        )}
