...
function RegisterHelm({ addAnotherHelmWorkflow }) {
  ...
  const [clickedRegisterHelm, setClickedRegisterHelm] = useState();
  ...
  const parsedSailNumber = parseInt(sailNumber);
  ...
  useEffect(() => {
    const processHelmResult = () => {
      ...
      updateAppState(({ registered, results, oods, ...state }) => {
        if (
          registered.find(
            (prev) =>
              HelmResult.getId(prev) === HelmResult.getId(newRegisteration)
          ) ||
          results.find(
            (prev) =>
              HelmResult.getId(prev) === HelmResult.getId(newRegisteration)
          ...
          return {
            ...
            registered: [...registered, newRegisteration].sort(
              (helmA, helmB) => {
                const helmAPI = services.getLatestHelmPersonalHandicap(
                  HelmResult.getHelmId(helmA)
                );
                const helmBPI = services.getLatestHelmPersonalHandicap(
                  HelmResult.getHelmId(helmB)
                );
                return helmBPI > helmAPI || helmBPI === undefined ? -1 : 1;
              }
            ),
          };
        }
      ...
      if (!addAnotherHelmWorkflow) {
        ...
        //races/2024-12-01/1/register
        navigateTo(
          `../../../races/${getURLDate(
            race.getDate()
          )}/${race.getNumber()}/registerAnother/${HelmResult.getHelmId(
            newRegisteration
          )}`,
          { replace: true }
        );
    ...
    if (clickedRegisterHelm && !isNaN(parsedSailNumber)) {
      processHelmResult();
    }
  }, [
  ...
  useEffect(() => {
    ...
    if (!selectedHelm) {
      setHelmsIndex(
        services.indexes.getHelmsIndex(excludedHelmIds, appState.newHelms)
      );
    }
  }, [appState.results]);
      ...
      <form onSubmit={(evt) => evt.preventDefault()}>
        ...
        >
          ...
          >
            ...
            {selectedClubMember && (
              <NewHelm onNewHelm={onNewHelm} clubMember={selectedClubMember} />
            )}
              ...
              <>
                <Autocomplete
                  ...
                  handleSelectedItemChange={(item) => {
                    if (item !== undefined) {
                      const sailNumber = !isNaN(parseInt(item))
                        ? parseInt(item)
                        : item.sailNumber;
            ...
            {(sailNumber !== undefined || showCommit) && (
              <GreenButton
                disabled={true}
                onClick={() => setClickedRegisterHelm(true)}
