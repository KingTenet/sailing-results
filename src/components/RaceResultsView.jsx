...
function getDimensionValue(dimension, result, sct, correctedLaps) {
    switch (dimension) {
            ...
            return result.isValidFinish()
                ? formatMinutesSeconds(
                      secondsToMinutesSeconds(
                          result.getPersonalCorrectedFinishTime(),
                      ),
                  )
                : "DNF";
        ...
        case "FINISH_TIME":
            return result.isValidFinish()
                ? formatMinutesSeconds(
                      secondsToMinutesSeconds(result.getFinishTime()),
                  )
                : "DNF";
        case "LAPS":
...
export function RaceResultsView({
    ...
    const [
        ...
        SCT,
        isPursuitRace,
    ] = useSortedResults(results, race, seriesRace);
...
function getPursuitStartTimes(results, race, raceLengthSeconds) {
    if (
        !results.length ||
        results.some(
            (result) => Race.getId(result.getRace()) !== Race.getId(race),
        )
    ) {
        return [];
    }
