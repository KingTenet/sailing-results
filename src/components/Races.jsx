    ...
    return (
        <>
            <Flex
                ...
                onClick={onClick}
            >
                <Grid templateColumns="repeat(6, 1fr)" gap={5} width={"100%"}>
                    <RaceDimension
                        colSpan={1}
                        paddingLeft="5px"
                    >{`${getURLDate(raceDate).replace(/-/g, "/")}`}</RaceDimension>
                    <RaceDimension
                        colSpan={1}
                        paddingLeft="5px"
                    >{`${formatRaceNumber(raceNumber)} race`}</RaceDimension>
                    <GridItem colSpan="4">
                        <Flex
                            direction="row"
...
function ImmutableRaceListItem({
    raceDate,
    raceNumber,
    raceResults = [],
    wonBy = [],
...
function RacesView({ races, ...props }) {
    ...
    const registeredByRace = mapGroupBy(appState.registered, [
            ...
            {races.map((race) => (
                <ListItem key={Race.getId(race)}>
                    <RaceListItem
                        ...
                        onClick={() =>
                            navigateTo(
                                `${getURLDate(race.getDate())}/${race.getNumber()}`,
                            )
                        }
                    />
                </ListItem>
...
function ImmutableRace({ race }) {
    ...
    const [
        ...
        SCT,
        isPursuitRace,
    ] = useSortedResults(undefined, race) || [];
            ...
            {Boolean(storesToSync.length) && (
                <Box margin="10px">
                    <Alert status="warning">
                        <AlertIcon />
                        <AlertTitle mr={2}>
                            Completed races have not yet been uploaded.
                        </AlertTitle>
                    </Alert>
                </Box>
            )}
            {!Boolean(storesToSync.length) && (
                <Box margin="10px">
                    <Alert status="success">
                        <AlertIcon />
                        <AlertTitle mr={2}>
                            All edited races have been uploaded successfully.
                        </AlertTitle>
                    </Alert>
                </Box>
            )}
...
export default function Races({ editableOnly = false }) {
    ...
    const [[mutableRaces, immutableRaces]] = useState(() =>
        services.getRaces(services.isLive),
    );
    const latestImmutableRaceDate = immutableRaces
        .sort((raceA, raceB) => raceB.sortByRaceAsc(raceA))
        .at(0)
        .getDate();
    ...
    const lastRaces = previousRaces.filter(
        (race) =>
            previousRaces.at(-1).getDate().getTime() ===
            race.getDate().getTime(),
    );

    const editedRaces = immutableRaces.filter((race) =>
        services.isRaceEditableByUser(race),
    );
    ...
    const todaysFinishedRaces = immutableRaces
        ...
        .filter(
            (race) =>
                race.getDate().getTime() === firstRaceToday.getDate().getTime(),
        );

    const immutableNotEdited = immutableRaces.filter(
        (race) => !editedRaces.includes(race),
    );

    const activeRaces = groupBy(
        [...appState.results, ...appState.oods, ...appState.registered],
        [HelmResult.getRaceId],
    )
        .map(([id]) => Race.fromId(id))
        .filter((race) =>
            services.isRaceMutable(race.getDate(), race.getNumber()),
        );

    const todaysActiveRaces = activeRaces.filter(
        (race) =>
            race.getDate().getTime() === firstRaceToday.getDate().getTime(),
    );

    const oldActiveRaces = activeRaces.filter(
        (race) =>
            race.getDate().getTime() !== firstRaceToday.getDate().getTime(),
    );
        ...
        <>
            <Flex direction="column" padding="5px">
                ...
                {Boolean(lastRaces.length) &&
                    (!showNextEditableRace || appState.adminMode) && (
                        <RacesCard>
                        ...
                        <RacesCard>
                            ...
                            <Box margin="10px">
                                <Alert status="warning">
                                    <AlertIcon />
                                    <AlertTitle mr={2}>
                                        Old active race results have not been
                                        committed.
                                    </AlertTitle>
                                </Alert>
                            </Box>
                    ...
                    <RacesCard>
                        ...
                        <Box margin="10px">
                            <Alert status="warning">
                                <AlertIcon />
                                <AlertTitle mr={2}>
                                    Active race results have not been committed.
                                </AlertTitle>
                            </Alert>
                        </Box>
                    </RacesCard>
                ...
                {!Boolean(activeRaces.length) && <StatusWrapper />}
            </Flex>
        </>
    );
}
