...
export class AutoMap extends Map {
    ...
    upsert(obj, transform = (prev, obj, key) => obj) {
        ...
        if (!this.has(key) && this.getDefaultValue !== undefined) {
            return this.set(
                key,
                transform(this.getDefaultValue(obj, key), obj, key),
            );
        }
...
export function groupBy(arr, allGroupFs, transform = (arr) => arr) {
    const groupFs =
        typeof allGroupFs === "function" ? [allGroupFs] : [...allGroupFs];
    const groupF = groupFs.reverse().pop();
    if (!groupF) {
        return transform(arr);
    }
    const groupResults = groupBySingle(arr, groupF);
    return groupResults.map(([groupId, results]) => [
        groupId,
        groupBy(results, [...groupFs].reverse(), transform),
    ]);
}
...
export function mapGroupBy(arr, allGroupFs, transform = (arr) => arr) {
    ...
    const groupResults = groupBySingle(arr, groupF);
    return new Map(
        groupResults.map(([groupId, results]) => [
            groupId,
            mapGroupBy(results, [...groupFs].reverse(), transform),
        ]),
    );
}
...
export const promiseSleep = (timeout) =>
...
export function assertType(obj, Type) {
    if (typeof Type === "string") {
        ...
        if (typeof obj !== Type) {
            throw new Error(
                `Object has type:${typeof obj} but should be type:${Type}`,
            );
        }
...
export function parseBoolean(input) {
    return !Boolean(
        !input ||
            (typeof input === "string" && input.toLowerCase() === "false"),
    );
}
...
export async function getGoogleSheetDoc(sheetId, clientEmail, privateKey) {
    ...
    try {
        ...
        if (err?.response?.status === 403) {
            throw new Error(
                `Check that you have granted permission to ${clientEmail} to access the google sheet ${sheetId}`,
            );
        }
        throw err;
    }
...
export async function limitSheetsRequest(req) {
    ...
    do {
        try {
            ...
            console.log(
                "Google Sheets API requests quota exceeded... waiting 60 seconds",
            );
...
export function getSheetIdFromURL(url) {
    ...
    if (!matches || !matches[1]) {
        throw new Error(
            "Invalid URL format, google sheet ID should match regex https://docs.google.com/spreadsheets/d/([^/]+)/.*$",
        );
    }
    return matches[1];
}

export async function getAllCellsFromSheet(
    sheetId,
    auth,
    sheetName,
