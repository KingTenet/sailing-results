...
export class Stores {
    constructor(auth, raceResultsSheetId, readOnly, forceCacheRefresh) {
        ...
        this.auth = auth;
        this.promiseMetaStore = RemoteStore.retryCreateRemoteStore(
    ...
    async getStoreLastUpdated() {
        try {
            ...
            return parseISOString(
                (await this.metaStore.getAllRows())[0]["Last Updated"],
            );
        } catch (err) {
            console.log(err);
    ...
    async createStoreAndProcessResults() {
        ...
        const promiseNewMembers = createStoreWrapper(
            "New Membership",
            this.raceResultsDocument,
            this,
            ClubMember,
        );
        const promiseHelms = createStoreWrapper(
            "Helms",
            this.raceResultsDocument,
            this,
            Helm,
        ...
        const promiseRYAClasses = createStoreWrapper(
            "RYA Full List",
            this.raceResultsDocument,
            this,
            BoatClass,
        );
        const promiseClubClasses = createStoreWrapper(
            "Club Handicaps",
            this.raceResultsDocument,
            this,
            BoatClass,
        );
        const promiseSeriesRaces = createStoreWrapper(
            "Seasons/Series",
            this.raceResultsDocument,
            this,
            SeriesRace,
        ...
        if ([...new Set(allClubMemberIds)].length !== allClubMemberIds.length) {
            throw new Error(
                "Duplicate helms exist in new members and existing members",
            );
        }
        ...
        this.ryaClasses = mapGroupBy(ryaClasses.all(), [
            BoatClass.getClassName,
        ]);
        this.clubClasses = mapGroupBy(clubClasses.all(), [
            BoatClass.getClassName,
        ]);
        ...
        const promiseOODs = createStoreWrapper(
            "OODs",
            this.raceResultsDocument,
            this,
            HelmResult,
        ...
        const promisePursuitResults = createStoreWrapper(
            "Pursuit Race Results",
            this.raceResultsDocument,
            this,
            Result,
            (storeResult) => this.deserialiseResult(storeResult),
        );
        const promiseFleetResults = createStoreWrapper(
            "Fleet Race Results",
            this.raceResultsDocument,
            this,
            Result,
        ...
        const [oods, pursuitResults, fleetResults] = await Promise.all([
            promiseOODs,
            promisePursuitResults,
            promiseFleetResults,
        ]);
        this.oods = oods;
    ...
    getStoresNames() {
        return this.getStores().map((store) => store.storeName);
    }
    ...
    async syncroniseRYA() {
        if (!this.readOnly) {
            const reportGenerator = new RYAReportGenerator(this);
            try {
    ...
    async syncroniseStore(storeName) {
        const store = this.getStores().find(
    ...
    getBoatClassForRace(boatClassName, race) {
        ...
        return BoatClass.getBoatClassesForRace(
            race,
            [ryaClasses],
            [clubClasses],
        ).get(boatClassName);
    ...
    deserialiseResult(storeResult, newHelms = [], Type = Result) {
        return Type.fromStore(
            ...
            (boatClassName, race) =>
                this.getBoatClassForRace(boatClassName, race),
        );
    }
    ...
    getHelmFromHelmId(helmId, newHelms = []) {
        ...
        const newHelm = newHelms.find(
            (newHelm) => Helm.getId(newHelm) === helmId,
        );
        if (!newHelm) {
            throw new Error(
                `Helm ${helmId} didn't exist in store or new helms`,
            );
        }
        return newHelm;
    }
    ...
    getRaceFinish(race) {
        ...
        return this.raceFinishes.find(
            (raceFinish) => Race.getId(raceFinish) === raceId,
        );
    }
    ...
    processResults(tranformResults) {
        const [
            ...
            raceFinishes,
            allCorrectedResults,
            allSeriesRacesByRace,
            helmResultsByRaceAsc,
        ] = Stores.processResultsStatic(
            this.oods.all(),
    ...
    static processResultsStatic(
        allOODs,
        allPursuitResults,
        allFleetResults,
        allSeriesRaces,
        transformResults = (results) => results,
        previousRaceFinishes = [],
        previousCorrectedResults = [],
    ) {
        console.log(
            `Started processing: OODs:${allOODs.length}, Fleet results:${allFleetResults.length}, Pursuit Results:${allPursuitResults.length} Series Races:${allSeriesRaces.length}`,
        );
        ...
        for (let [race, raceResults] of Race.groupResultsByRaceAsc(
            transformResults(allPursuitResults),
        )) {
            raceFinishes.upsert(
        ...
        const allCorrectedResults = [
            ...previousCorrectedResults.sort(Result.sortByRaceAsc),
        ];

        const initialResultsByHelm = new mapGroupBy(allCorrectedResults, [
            Result.getHelmId,
        ]);
        const helmResultsByRaceAsc = new AutoMap(
            Result.getHelmId,
            (result, helmId) => initialResultsByHelm.get(helmId) || [],
        );

        for (let [race, raceResults] of Race.groupResultsByRaceAsc(
            transformResults(allFleetResults),
        )) {
            ...
            correctedResults.forEach((result) =>
                helmResultsByRaceAsc.upsert(result, (arr) => [...arr, result]),
            );
            allCorrectedResults.push(...correctedResults);
        }

        const allSeriesRacesByRace = mapGroupBy(allSeriesRaces, [
            (seriesRace) => Race.getId(seriesRace.getRace()),
        ]);
        [...allSeriesRacesByRace].forEach(([, seriesRaces]) => {
            ...
            if (seriesRaces.length > 1) {
                ...
                if (inconsistent) {
                    throw new Error(
                        `Series races: ${SeriesRace.getId(
                            inconsistent,
                        )} and ${SeriesRace.getId(
                            firstRace,
                        )} have inconsistent race types (fleet/pursuit)`,
                    );
                }
            }
        });
        ...
        console.log(
            `Finished processing results in ${Math.round(Date.now() - started)} ms`,
        );
...
class Indexes {
    ...
    setBoatCounts(results = []) {
        this.boats = new Map(
            Indexes.getBoatCounts(this.getAllResults(results)),
        );
        this.boatsByHelm = Indexes.getBoatsForHelm(this.getAllResults(results));
    }

    setSailNumberCounts(results = []) {
        this.sailNumbersByHelmBoat = Indexes.getSailNumbersForHelmBoat(
            this.getAllResults(results),
        );
        this.sailNumbersByBoat = Indexes.getSailNumbersForBoat(
            this.getAllResults(results),
        );
    }
    ...
    getHelmsIndex(excludedHelms = [], newHelms = []) {
        ...
        const scoresByHelmId = new Map(
            [...this.helms].map(([helmId, [count, time]]) => [
                helmId,
                getScore(count, time),
            ]),
        );
    ...
    getBoatIndexForHelmRace(helm, race) {
        const ryaClasses = [...this.stores.ryaClasses].map(
            ([, classes]) => classes,
        );
        const clubClasses = [...this.stores.clubClasses].map(
            ([, classes]) => classes,
        );

        const allClassesForYearByClassName = BoatClass.getBoatClassesForRace(
            race,
            ryaClasses,
            clubClasses,
            true,
        );
        const allClassesForYear = [...allClassesForYearByClassName].map(
            ([, boatClass]) => boatClass,
        );
        ...
        const sortCountsByScore = (boatCounts) => {
            return boatCounts
                .map(([className, values]) => [
                    allClassesForYearByClassName.get(className),
                    values,
                ])
                .filter(([boatClass]) => boatClass)
                .map(([boatClass, [count, time]]) => [
                    boatClass,
                    [count, new Date(time), getScore(count, time)],
                ])
                .sort(([, [, , scoreA]], [, [, , scoreB]]) => scoreB - scoreA);
        };

        // Sorted by score desc
        const boatsForHelmByScore = sortCountsByScore(
            this.boatsByHelm.get(Helm.getId(helm)) || [],
        );
        const uniqueBoatClassesForHelm = new Set(
            boatsForHelmByScore.map(([boat]) => boat.getClassName()),
        );
        const allBoatsByScore = sortCountsByScore([...this.boats]).filter(
            ([boat]) => !uniqueBoatClassesForHelm.has(boat.getClassName()),
        );
        const allUniqueBoatClasses = new Set(
            allBoatsByScore.map(([boat]) => boat.getClassName()),
        );

        const remainingClasses = allClassesForYear.filter(
            (boat) =>
                !uniqueBoatClassesForHelm.has(boat.getClassName()) &&
                !allUniqueBoatClasses.has(boat.getClassName()),
        );
    ...
    getSailNumberIndexForHelmBoat(helm, boat) {
        ...
        const getSailNumbersByBoat = (helm, boat) => {
            const sailNumbersByBoat =
                this.sailNumbersByHelmBoat.get(Helm.getId(helm)) ||
        ...
        return new SearchIndex(
            sailNumbers.map(([sailNumber, [count, date, score]], key) => ({
                sailNumber: `${sailNumber}`,
                date,
                count,
                rank: (sailNumbers.length - key) / sailNumbers.length,
                score,
            })),
            "sailNumber",
    ...
    static getSailNumberCounts(boatResults) {
        const sailNumberCounts = new AutoMap(
            (result) => result.getSailNumber(),
            () => [0, 0],
        );
        for (let result of boatResults) {
            sailNumberCounts.upsert(result, ([count, last]) => [
                count + 1,
    ...
    static getBoatCounts(results) {
        const boatCounts = new AutoMap(
            (result) => result.getBoatClass().getClassName(),
            () => [0, 0],
        );
        for (let result of results) {
            boatCounts.upsert(result, ([count, last]) => [
                count + 1,
                Math.max(last, result.getRace().getDate().getTime()),
            ]);
        }
    ...
    static getHelmCounts(results) {
        ...
        for (let result of results) {
            helmCounts.upsert(result, ([count, last]) => [
                count + 1,
                Math.max(last, result.getRace().getDate().getTime()),
            ]);
        }
...
export class StoreFunctions {
    ...
    helmIsClubMember(helm) {
        return (
            this.stores.clubMembers.has(Helm.getId(helm)) ||
            this.stores.newMembers.has(Helm.getId(helm))
        );
    }

    async commitNewHelmsForResults(race, results, oods, newHelms) {
        await promiseSleep(0);
        const [raceResults, raceOODs] = this.getResultsOODsForRace(
            race,
            results,
            oods,
        );
    ...
    getResultsOODsForRace(race, results, oods) {
        const raceResults = results.filter(
            (result) => Result.getRaceId(result) === Race.getId(race),
        );
        const raceOODs = oods.filter(
            (ood) => Result.getRaceId(ood) === Race.getId(race),
        );
    ...
    isRaceEditableByUser(race) {
        return (
            this.superUser ||
            (this.editableRaceDate &&
                race.getDate().getTime() === this.editableRaceDate.getTime())
        );
    }

    isPursuitRace(race) {
        assertType(race, Race);
        const seriesRacesForRace = this.stores.allSeriesRacesByRace.get(
            Race.getId(race),
        );
    ...
    isRaceMutable(raceDate, raceNumber) {
        ...
        const immutableResults = Race.groupResultsByRaceAsc([
            ...this.stores.results.all(),
            ...this.stores.pursuitResults.all(),
    ...
    getRaces(exludeFutureMutableRaces) {
        ...
        const immutableRaces = Race.groupResultsByRaceAsc([
        ...
        const mutableRaces = this.stores.seriesRaces
            ...
            .filter(
                (race) =>
                    !immutableRaces.find(
                        (immutable) =>
                            Race.getId(race) === Race.getId(immutable),
                    ),
            )
            .sort((a, b) => a.sortByRaceAsc(b))
            .filter(
                (race) =>
                    !exludeFutureMutableRaces || !tomorrowsRace.isBefore(race),
            );
    ...
    createHelmFromClubMember(clubMember, gender, noviceInFirstRace) {
        const newHelm = Helm.fromClubMember(
            clubMember,
            gender,
            noviceInFirstRace,
        );
    ...
    deserialiseRegistered(storedRegistered, newHelms = []) {
        ...
        // missing helm/boat
        // existing result
        const registered = this.stores.deserialiseResult(
            storedRegistered,
            newHelms,
            MutableRaceResult,
        );
    ...
    deserialiseOOD(storeOOD, newHelms = []) {
        ...
        // missing helm/boat
        // existing result
        const ood = this.stores.deserialiseResult(
            storeOOD,
            newHelms,
            HelmResult,
        );
    ...
    getLatestHelmPersonalHandicap(helmId) {
        if (this.stores.helmResultsByRaceAsc.has(helmId)) {
            const latestFleetResult = this.stores.helmResultsByRaceAsc
                .get(helmId)
                .at(-1);
            const [ph, pi] =
                (latestFleetResult &&
                    latestFleetResult.getRollingHandicapsAtRace()) ||
                [];
            return pi;
        }
    }
    ...
    getRaceFinishForResults(
        ...
        if (
            !mutableResults.length &&
            !mutableOODs.length &&
            !mutableSeriesRaces.length
        ) {
        ...
        if (
            mutableResults.some(
                (result) => Race.getId(result.getRace()) !== raceId,
            )
        ) {
            throw new Error("All results must be from same race");
        }

        const mutableFleetResults =
            mutableResults.length && Race.isPursuitRace(mutableResults)
                ? []
                : mutableResults;
        const mutablePursuitResults =
            mutableResults.length && Race.isPursuitRace(mutableResults)
                ? mutableResults
                : [];
        ...
        const [seriesPoints, raceFinishes, allCorrectedResults] =
            Stores.processResultsStatic(
                [...storedOODs, ...mutableOODs],
