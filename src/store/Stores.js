...
export class Stores {
  ...
  getStatus() {
    return this.getStores().reduce(
      (acc, store) => ({ ...acc, [store.storeName]: store.storesInSync() }),
      {}
    );
  }
    ...
    [...allSeriesRacesByRace].forEach(([, seriesRaces]) => {
      ...
      if (seriesRaces.length > 1) {
        ...
        const inconsistent = seriesRaces.find(
          (seriesRace) => seriesRace.isPursuit() !== firstRace.isPursuit()
        );
        if (inconsistent) {
          throw new Error(
...
class Indexes {
  ...
  static getBoatsForHelm(results) {
    return mapGroupBy(results, [HelmResult.getHelmId], Indexes.getBoatCounts);
  }
...
export class StoreFunctions {
  ...
  getRaces(exludeFutureMutableRaces) {
    ...
    const mutableRaces = this.stores.seriesRaces
      ...
      .filter(
        (race) =>
          !immutableRaces.find(
            (immutable) => Race.getId(race) === Race.getId(immutable)
          )
      )
      .sort((a, b) => a.sortByRaceAsc(b))
      .filter(
        (race) => !exludeFutureMutableRaces || !tomorrowsRace.isBefore(race)
      );
  ...
  getLatestHelmPersonalHandicap(helmId) {
    if (this.stores.helmResultsByRaceAsc.has(helmId)) {
      const latestFleetResult = this.stores.helmResultsByRaceAsc
        ...
        .at(-1);
      const [ph, pi] =
        (latestFleetResult && latestFleetResult.getRollingHandicapsAtRace()) ||
        [];
      return pi;
    }
  }
  ...
  getRaceFinishForResults(
    ...
    if (
      mutableResults.some((result) => Race.getId(result.getRace()) !== raceId)
    ) {
