      ...
      rowCells
        .map((value, key) =>
          this.pad(
            value,
            " ",
            key < this.rowHeaders.length ? this.rowHeadersWidth : columnWidth
          )
        )
        .join(` ${this.columnSep} `) +
      ` ${this.columnSep}`
...
export default class SeriesPoints extends Series {
  ...
  validateRaceFinishes() {
    ...
    if (
      !this.raceFinishes.map(Race.getId).every((id) => seriesRacesById.has(id))
    ) {
      ...
      groupBy(allPointResults, HelmResult.getHelmId).map(
        ([, helmPointResults]) =>
          helmPointResults
            ...
            .map((pointResult, key) =>
              ResultPoints.fromPoints(pointResult, Boolean(key < racesToCount))
            )
      )
    );
  }
  ...
  getRacePointsByHelmBoat(allResultPoints) {
    ...
    const numRaceEntrants = mapGroupBy(
      ...
      [HelmResult.getRaceId],
      (allPoints) =>
        allPoints.reduce((acc, points) => acc + Boolean(points.racePoints), 0)
    );
