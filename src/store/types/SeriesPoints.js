...
class AsciiTable {
    constructor(columnHeaders, rowHeaders, cells) {
        ...
        this.columnSize = Math.ceil(
            (this.totalColumnSize - this.rowHeadersWidth * rowHeaders.length) /
                this.cells[0].length,
        );
    ...
    pad(str = "", char, length) {
        return [...str, ...new Array(length).fill(char)]
            .join("")
            .slice(0, length);
    }
    ...
    getTable() {
        ...
        let headerRowLength;
        const rowSep = () =>
            [...new Array(headerRowLength).fill(this.rowSep)].join("");

        this.columnHeaders.forEach((columnHeader) => {
            const headerRow = this.getRow(
                [...padRowHeaders, ...columnHeader],
                this.columnSize - 3,
            );
        ...
        const rows = this.cells.reduce(
            (prevRows, row, key) => [
                ...prevRows,
                this.getRow(
                    [
                        ...this.rowHeaders.map((rowHeader) => rowHeader[key]),
                        ...row.map((num) => (num === undefined ? "" : num)),
                    ],
                    this.columnSize - 3,
                ),
                rowSep(),
...
export default class SeriesPoints extends Series {
    constructor(series, seriesRaces, raceFinishes) {
        ...
        raceFinishes.forEach((raceFinish) =>
    ...
    validateSeriesRaces() {
        ...
        if (
            !this.seriesRaces
                .map(SeriesRace.getSeriesId)
                .every((id) => id === seriesId)
        ) {
    ...
    validateRaceFinishes() {
        const seriesRacesById = new Map(
            groupBy(this.seriesRaces, [SeriesRace.getRaceId]),
        );
        if (
            !this.raceFinishes
                .map(Race.getId)
                .every((id) => seriesRacesById.has(id))
        ) {
            throw new Error(
                "All race finishes must have an associated series race",
            );
        }
    }
    ...
    getClassHandicapRacesToCount(date) {
        return SeriesPoints.getRacesToCount(this.getFinishesWithResults(date));
    }
            ...
            groupBy(allPointResults, ResultPoints.getPersonId).map(
                ([, helmPointResults]) =>
                    helmPointResults
                        ...
                        .map((pointResult, key) =>
                            ResultPoints.fromPoints(
                                pointResult,
                                Boolean(key < racesToCount),
                            ),
                        ),
            ),
        );
    ...
    getRacePointsByPersonBoat(allResultPoints) {
        ...
        // columnHeaders
        const sortedRaces = this.seriesRaces
            .map((sr) => sr.getRace())
        ...
        const numRaceEntrants = mapGroupBy(
            ...
            (allPoints) =>
                allPoints.reduce(
                    (acc, points) => acc + Boolean(points.racePoints),
                    0,
                ),
        );
    ...
    summarizeByPersonalHandicap(date = new Date()) {
        ...
        if (!this.allPersonalHandicapPoints) {
            this.allPersonalHandicapPoints =
                this.getPersonalHandicapPoints(date);
        }
    ...
    summarizeHelms(allResultPoints) {
        groupBy(allResultPoints, HelmResult.getHelmId)
            .map(([helmId, resultsPoints]) => [
                helmId,
                ResultPoints.aggregate(resultsPoints),
            ])
            .sort(([, a], [, b]) => a - b)
    ...
    seriesTable(allResultPoints) {
        const allResultsPointsSortedByRace = allResultPoints.sort(
            HelmResult.sortByRaceAsc,
        );
        const firstRacePointsOfSeries = allResultsPointsSortedByRace
            .at(0)
            .getRace();
        const allResultsByHelm = mapGroupBy(
            allResultPoints,
        ...
        const allBoatsByHelm = groupBy(
            ...
            (points) =>
                points && points.length
                    ? [
                          ResultPoints.getPersonalHandicapForRace(
                              points.at(0),
                              firstRacePointsOfSeries,
                          ),
                          ResultPoints.getBoatClass(points.at(0))?.getPY(),
                      ]
                    : undefined,
        ...
        let columnHeaders = columnIds.map((raceId) =>
            Race.fromId(raceId).prettyPrintTable(),
        );

        let rowHeaders = allBoatsByHelm
            .reduce(
                (acc, [helmId, classes]) => [
                    ...acc,
                    ...classes.map(([className, [PH, classPY]]) => [
                        helmId,
                        className,
                        PH,
                        classPY,
            ...
            .sort(([helmA, classA], [helmB, classB]) => {
                return allResultsByHelm.get(helmA) !==
                    allResultsByHelm.get(helmB)
                    ? allResultsByHelm.get(helmA) - allResultsByHelm.get(helmB)
                    : classA - classB;
            });

        const rowHeadersHelm = rowHeaders.reduce(
            (acc, [helmId]) => [...acc, helmId === acc.at(-1) ? "" : helmId],
            [],
        );
        const rowHeadersBoat = rowHeaders.map(
            ([helmId, className, PHPI = [], classPY]) =>
                className
                    ? [
                          helmId,
                          className,
        ...
        for (let [rowKey, [helmId, className]] of rowHeaders.entries()) {
            ...
            for (let [columnKey, raceId] of columnIds.entries()) {
                ...
                const helmPoints = racePoints
                    ? racePoints.get(className)
                    : undefined;
