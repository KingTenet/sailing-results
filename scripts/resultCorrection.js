...
function calculateSCTFromRaceResults(raceResults) {
    ...
    // console.log(finishers.map((a) => a.getHelm().name));
    // console.log(finishers.map((a) => a.getClassCorrectedTime(raceMaxLaps) / raceMaxLaps));
    // console.log(finishTimes);

    const ACT = average(finishTimes.slice(-resultsToCountForACT));

    console.log(`ACT: ${ACT / raceMaxLaps} using ${resultsToCountForACT} finishers`);

    const resultsWithin105ACT = finishTimes.filter((time) => time < (ACT * 1.05));

    const SCT = average(resultsWithin105ACT);
    console.log(`SCT: ${SCT / raceMaxLaps} using ${resultsWithin105ACT.length} finishers and ${raceMaxLaps} laps`);

    return [SCT, raceMaxLaps];
}
