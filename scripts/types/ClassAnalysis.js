...
export default class ClassAnalysis extends StoreObject {
    constructor(raceFinish, boatClass, metadata) {
        ...
        this.finishers = ClassAnalysis.getFinishersForClass(
            raceFinish,
            boatClass,
        );
    }
    ...
    static getId(classAnalysis) {
        ...
        return generateId("ClassAnalysis", [
            BoatClass.getId(classAnalysis.boatClass),
            MutableRaceFinish.getId(classAnalysis.raceFinish),
        ]);
    }

    getPIForResult(result) {
        return calculatePIFromPersonalHandicap(
            this.boatClass.getPY(),
            result.getPersonalHandicapFromRace(),
        );
    }

    getFinishersWithin105SCT() {
        return this.finishers.filter(
            (result) => this.getPIForResult(result) < MAX_PI_WITHIN_105_SCT,
        );
    }
    ...
    static getTotalPH(finishers) {
        return finishers.reduce(
            (acc, result) => acc + result.getPersonalHandicapFromRace(),
            0,
        );
    }
    ...
    toStore() {
        ...
        return {
            ...
            "RYA Approved SCT": this.raceFinish.ryaApprovedSCT
                ? "TRUE"
                : "FALSE",
            ...super.toStore(this),
        };
