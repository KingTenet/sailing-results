...
const ouptutSheetURL =
    "https://docs.google.com/spreadsheets/d/15CESU67Qy34N_rzirxRQnSvbcSosxNXAB-TZNvYLP9k/edit#gid=684050272";
...
async function run(forceRefresh = "true") {
    ...
    const outputDocument = getGoogleSheetDoc(
        outputSheetId,
        auth.clientEmail,
        auth.privateKey,
    );
    ...
    await outputClassHandicaps(
        stores,
        "Current Class Handicaps",
        outputDocument,
    );
}

async function outputClassHandicaps(stores, sheetName, sheetDoc) {
    const allCorrectedResults = flatten(
        stores.raceFinishes.map((raceFinish) =>
            raceFinish.hasResults() ? raceFinish.getCorrectedResults() : [],
        ),
    );

    const getClassesCountFromDate = (date) => {
        const raceDate = new Race(date, 1);
        const results = allCorrectedResults.filter(
            (result) => !result.getRace().isBefore(raceDate),
        );
        return mapGroupBy(
            results,
    ...
    const now = new Date();
    const classesCount12Month = getClassesCountFromDate(
        new Date(now.getFullYear() - 1, now.getMonth(), now.getDate()),
    );
    const classesCount48Month = getClassesCountFromDate(
    ...
    const allCurrentClasses = BoatClass.getBoatClassesForRace(
        todaysRace,
        ryaClasses,
        clubClasses,
        EXCLUDE_DEPRECATED,
    ...
    const classHandicapsStore = await StoreWrapper.create(
        false,
        sheetName,
        sheetDoc,
        stores,
    ...
    for (let [className, boatClass] of [...allCurrentClasses]) {
        classHandicapsStore.add(
            BoatClassRaces.fromBoatClassRaces(
                boatClass,
                classesCount12Month.get(className) || 0,
                classesCount48Month.get(className) || 0,
            ),
        );
    }
