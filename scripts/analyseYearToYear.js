...
const sourceResultsURL =
    "https://docs.google.com/spreadsheets/d/1Q5fuKvddf8cM6OK7mN6ZfnMzTmXGvU8z3npRlR56SoQ/edit#gid=771994490";
const analysisSheetURL =
    "https://docs.google.com/spreadsheets/d/1Qg8-A0SzykgQROtDDg3H1SKkxbrZ7lF7Kw8PdeOWoZY/";
...
async function outputSeriesHelms(stores, sheetName, sheetDoc) {
    const seriesResults = flatten(
        flatten(
            stores.seriesPoints.map(([, seriesPoints]) =>
                seriesPoints.raceFinishes
                    .map(
                        (raceFinish) =>
                            raceFinish.hasResults() &&
                            raceFinish
                                .getCorrectedResults()
                                .map((result) => [seriesPoints, result]),
                    )
                    .filter(Boolean),
            ),
        ),
    ...
    const racesByHelm = mapGroupBy(
        allResults,
        [Result.getHelmId],
        (results) =>
            results.sort(HelmResult.sortByRaceAsc).map(Result.getRaceId)[0],
    );

    const seriesHelms = groupBy(seriesResults, [
        ([seriesPoints]) => Series.getId(seriesPoints),
        ([, result]) => HelmResult.getHelmId(result),
    ]);

    const seriesHelmsStore = await StoreWrapper.create(
        false,
        sheetName,
        sheetDoc,
        stores,
    ...
    for (let [seriesId, helmSeriesResults] of seriesHelms) {
        for (let [, seriesResults] of helmSeriesResults) {
            ...
            const isFirstSeries = results
                .map(Result.getRaceId)
                .includes(firstRace);

            seriesHelmsStore.add(
                HelmSeries.fromSeriesHelm(
                    Series.fromId(seriesId),
                    helm,
                    isFirstSeries,
                ),
            );
