...
const analysisSheetURL =
    "https://docs.google.com/spreadsheets/d/1IIDTZuA5qt67rjftgJe3Cs2oVwkJ6Tn0Hste9YYOGks/edit#gid=1997904671";
...
async function run(forceRefresh = "true") {
    ...
    await outputClassAnalysis(
        stores,
        "Class Analysis For Experienced Solo/Laser Helms",
        analysisDocument,
        tranformIncludeLaserSoloHelms(),
    );

    await outputCorrectedResults(
        stores,
        "Corrected Results For Experienced Solo/Laser Helms",
        analysisDocument,
        tranformIncludeLaserSoloHelms(),
    );
}

async function outputCorrectedResults(
    stores,
    sheetName,
    sheetDoc,
    tranformResults,
    ...
    const correctedResultsStore = await StoreWrapper.create(
        false,
        sheetName,
        sheetDoc,
        stores,
...
async function outputClassAnalysis(
    stores,
    sheetName,
    sheetDoc,
    tranformResults,
    ...
    const classAnalyses = flatten(
        groupBy(
            ...
            flatten(analysesByRace.map(([, classAnalysis]) => classAnalysis)),
        ),
    );

    const classAnalysisStore = await StoreWrapper.create(
        false,
        sheetName,
        sheetDoc,
        stores,
...
async function outputHeadToHead(stores, sheetName, sheetDoc, helmAId, helmBId) {
    ...
    const headToHeadResults = stores
        .getRaceFinishes()
        .filter((raceFinish) => raceFinish.hasResults())
        .map((raceFinish) => raceFinish.getClassCorrectedPointsByResult())
        .map((resultsByPosition) =>
            resultsByPosition.filter(([result]) =>
                [helmAId, helmBId].includes(Result.getHelmId(result)),
            ),
        )
        .filter((resultsForHelms) => resultsForHelms.length === 2) // Both helms were in the race
        .map((resultsForHelms) => {
            const getResultPositionForHelmId = (helmId) =>
                resultsForHelms.find(
                    ([result]) => Result.getHelmId(result) === helmId,
                ) || [];
...
function tranformIncludeLaserSoloHelms(
    ...
    return (results) =>
        Race.groupResultsByRaceAsc(
            ...
            .filter(([, results]) =>
                results.some((result) => result.finishCode.validFinish()),
            )
